...
HashTable<Key, Value>::HashTable(unsigned int sze, Key init, unsigned int (*f) (const Key)) {
	size = sze; 
	keys = new Key[size]; //creas var llaves, que es un objeto llave con el tamaño definido
	if(keys == NULL){ // si llaves está vacío
		throw OutOfMemory();
	}
	initialValue = init;
	for(unsigned int i = 0; i < size; i++){ //ciclo para recorrer todas las llaves 
		keys[i] = initialValue;  //asigna que todas las llaves tengan el valor inicial definido
	}
